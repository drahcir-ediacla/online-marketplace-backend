...
const loginUser = (req, res) => {
  passport.authenticate('local', (authErr, user, info) => {
    ...
    req.logIn(user, (loginErr) => {
      ...
      res.cookie('jwt', accessToken, { httpOnly: true, sameSite: 'none', secure: false, maxAge: 24 * 60 * 60 * 1000, path: '/' }); //maxAge is equivalent to 24 hours
...
const logoutUser = (req, res) => {
  if (req.isAuthenticated()) {
    ...
    const refreshToken = req.user.id;
      // Clear Passport session
      req.logout(() => {});  // Clear Passport session with an empty callback
      

    db.query('DELETE FROM refresh_tokens WHERE user_id = ?', [refreshToken], (deleteError, deleteResult) => {
      if (deleteError) {
